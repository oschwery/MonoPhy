        ...
        for (i in 1:length(solution)){
            ...
            if (length(taxa) == 0){  # display all if no taxon specified
                ...
                for (itx in 1:length(taxa)) {  # loop to go through vector of taxon names
                    nametax2 <- taxa[itx]  # display name of invaded taxon first
                    tmp <- solution[[i]]$IntruderTaxa[[taxa[itx]]]  # display invading taxa
        ...
        if (taxlevels > length(solution)){
            ...
            if (length(taxa) == 0){  # display all if no taxa specified
                ...
                for (itx in 1:length(taxa)) {  # loop to go through vector of taxon names
                    ...
                    tmp <- solution[[taxlevels]]$IntruderTaxa[[taxa[itx]]]  # display invading taxa
                    alltaxa2[[nametax2]] <- tmp
                }
                alltaxa[[nametax]] <- alltaxa2
            }
